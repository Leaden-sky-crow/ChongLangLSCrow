...
export default function EditPostPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (originalPost) {
        localStorage.setItem(`edit_post_${params.id}`, JSON.stringify({ formData, content }))
      }
    }, 1000)
    return () => clearTimeout(timer)
  }, [formData, content, originalPost, params.id])
  ...
  }, [content, formData, lastSavedContent, autoSaving, originalPost, params.id])
  ...
  const handleSubmit = async (status: 'draft' | 'pending') => {
    ...
    if (result?.error) {
      ...
      localStorage.removeItem(`edit_post_${params.id}`)
      router.push('/profile')
    }
    setLoading(false)
  }
